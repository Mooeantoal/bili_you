...
class PipePipeNativePlayerPage extends StatefulWidget {
  const PipePipeNativePlayerPage({Key? key}) : super(key: key);

  @override
  State<PipePipeNativePlayerPage> createState() => _PipePipeNativePlayerPageState();
}

class _PipePipeNativePlayerPageState extends State<PipePipeNativePlayerPage>
    with SingleTickerProviderStateMixin {
  static const MethodChannel _channel = MethodChannel('pipepipe_player');
  ...
  Future<dynamic> _handleMethodCall(MethodCall call) async {
    switch (call.method) {
      case 'onPlayerStateChanged':
        final bool isPlaying = call.arguments['isPlaying'];
  ...
  Future<void> _loadVideo() async {
    ...
    try {
      // 通过UAPI获取视频播放信息
      final videoInfoUrl = 'https://uapis.cn/api/v1/social/bilibili/videoinfo?bvid=$videoId';
      final response = await _dio.get(videoInfoUrl);
      
      if (response.statusCode == 200) {
        final data = response.data;
        // 这里应该从UAPI获取DASH或HLS URL
        // 暂时使用示例数据
        final dashUrl = 'https://example.com/video.mpd'; // 示例URL
        final Map<String, String> headers = {}; // 示例headers
      ...
      setState(() {
        errorMessage = '获取视频信息失败: ${e.message}';
      });
    } on PlatformException catch (e) {
      setState(() {
  ...
  Future<void> _setPlaybackSpeed(double speed) async {
    ...
    try {
      ...
      setState(() {
        errorMessage = '播放速度控制失败: ${e.message}';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('PipePipe原生播放器'),
      ),
      body: Column(
        children: [
          // 添加输入框和跳转按钮
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Slider(
                                  ...
                                  onChanged: (value) {
                                    _seekTo((value * _duration).toInt());
                                  },
                                ),
                              ),
                              Text(
                                '${Duration(milliseconds: _position).toString().split('.').first} / ${Duration(milliseconds: _duration).toString().split('.').first}',
